...
class CategoryCore extends ObjectModel {
    ...
    public function existsInShop($id_shop) {
     ...
     public static function getCatsAsOptLst() {
        $categories = array();
        $sql = "SELECT cl.id_category as id,cl.name,cl.link_rewrite,c.level_depth,cs.position FROM `" . _DB_PREFIX_ . "category_lang` as cl LEFT JOIN " . _DB_PREFIX_ . "category as c ON c.id_category=cl.id_category LEFT JOIN " . _DB_PREFIX_ . "category_shop as cs ON c.id_category=cs.id_category WHERE c.active=1 and c.level_depth='2' ORDER BY cs.position ASC";
        $cats = Db::getInstance()->executeS($sql);
        $categories = $cats;
        foreach ($cats as $key => $cat) {
            $sql = "SELECT cl.id_category as id,cl.name,cl.link_rewrite,c.level_depth,cs.position FROM `" . _DB_PREFIX_ . "category_lang` as cl LEFT JOIN " . _DB_PREFIX_ . "category as c ON c.id_category=cl.id_category LEFT JOIN " . _DB_PREFIX_ . "category_shop as cs ON c.id_category=cs.id_category WHERE c.active=1 and id_parent='" . $cat['id'] . "' and c.level_depth='3' ORDER BY cs.position ASC";
            $main_cats = Db::getInstance()->executeS($sql);
            $categories[$key]['sub_cats'] = $main_cats;
        }
        return $categories;
    }
