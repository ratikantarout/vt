                    ...
                    <div class="navbar-collapse collapse" id="main_navbar_items">
                        <ul class="nav navbar-nav">
                            {foreach from=$catoptlist key=catkey item=catlist}
                                {if $catlist['sub_cats']}
                                    <li class="dropdown ">
                                        <a title="{$catlist['name']}{l s=' - VisitorTrip'}" data-toggle="dropdown" class="dropdown-toggle text-uppercase" href="{$link->getCategoryLink($catlist['id'], $catlist['link_rewrite'])|escape:'html':'UTF-8'}"><span class="fa fa-star"></span>{$catlist['name']}<span class="caret"></span></a>
                                        <ul role="menu" class="dropdown-menu" {if $catlist['id']=="30"}style="max-height: 350px;overflow-y:scroll;"{/if}>
                                            {foreach from=$catlist['sub_cats'] key=subcatkey item=subcatlist}
                                                <li><a title="{$subcatlist['name']}{l s=' - VisitorTrip'}" href="{$link->getCategoryLink($subcatlist['id'], $subcatlist['link_rewrite'])|escape:'html':'UTF-8'}">{$subcatlist['name']}</a></li>   
                                                {/foreach}
                                        </ul>
                                    </li>
                                {else}
