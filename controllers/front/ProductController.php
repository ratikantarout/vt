...
class ProductControllerCore extends FrontController {
    ...
    public function initContent() {
        ...
        if (!$this->errors) {
            ...
            $nearByPlace = array();
//            p($accessories);die;
            $nearByPlace = $this->getNearByPlaces($this->product->id, $this->product->width, $this->product->height, "50");
           
            $cover_img= $this->product->getCover($this->product->id);
            $this->context->smarty->assign(array(
                'image_c'=>$cover_img['id_image'],
                'nearplaces' => $nearByPlace,
    ...
    private function getNearByPlaces($id, $lat, $lng, $distance = 20) {
        $haversinSql = $this->haversinSqlGen($lat, $lng, "p");
        $sql = "select p.id_product,i.id_image,p.width as lat,p.height as lng,p.ean13,p.reference,pl.name,pl.link_rewrite," . $haversinSql . " from " . _DB_PREFIX_ . "product as p INNER JOIN " . _DB_PREFIX_ . "product_lang as pl ON p.id_product=pl.id_product INNER JOIN " . _DB_PREFIX_ . "image as i ON p.id_product=i.id_product where i.cover='1' GROUP BY i.id_image  having distance <= " . $distance . " and p.id_product !='" . $id . "' ";
        return Db::getInstance()->executeS($sql);
    }

    public function haversinSqlGen($lat, $lng, $tablealias) {
        $sql = "ROUND(1.60934*3956*2*ASIN(SQRT(POWER(SIN((" . $lat . "-abs(" . $tablealias . ".width))*pi()/180 / 2),2) + COS(" . $lat . "*pi()/180)*COS(abs(" . $tablealias . ".width)*pi()/180)*POWER(SIN((" . $lng . "-" . $tablealias . ".height)*pi()/180/2),2))),1) as distance";
        return $sql;
    }
